## Навчальний гайд по проєкту для Junior Frontend
...
Читай цей файл послідовно — цього має вистачити, щоб впевнено орієнтуватися в коді.
...
> `.env.local` зазвичай не комітиться в Git, тому при клонуванні репозиторію його треба створити заново.
...
- замість цього кожен розробник налаштовує `.npmrc` локально, або токен задають через змінні оточення CI.

Якщо ти клонуватимеш репозиторій, де `.npmrc` не збережений:

1. Потрібно буде створити свій `.npmrc` з токеном FontAwesome (або замінити приватні пакети на відкриті).
2. Без цього `npm install` може впасти з помилкою доступу до приватного реєстру.
